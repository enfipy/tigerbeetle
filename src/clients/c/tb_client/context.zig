    ...
    return struct {
        ...
        pub fn init(
            ...
            context.io = IO.init(32, 0) catch |err| {
                ...
                return if (comptime builtin.target.os.tag == .windows) switch (err) {
...
const Locker = extern struct {
    ...
    fn futex_wake(state_ptr: *std.atomic.Value(u32), waiters: u32) void {
        if (comptime builtin.target.os.tag == .linux) {
            ...
            if (waiters == 1) {
                windows.ntdll.RtlWakeAddressSingle(&state_ptr.raw);
            } else {
                windows.ntdll.RtlWakeAddressAll(&state_ptr.raw);
            }
        } else {
