...
fn emit_enum(
    ...
    try buffer.print("typedef enum {s} {{\n", .{c_name});
...
fn emit_struct(
    ...
    try buffer.print("typedef struct {s} {{\n", .{c_name});
    ...
    inline for (stdx.type_fields(Type)) |field| {
        ...
        switch (@typeInfo(field.type)) {
            .array => |array| try buffer.print("[{d}]", .{array.len}),
            else => {},
        }
...
pub fn main() !void {
    ...
    inline for (type_mappings) |type_mapping| {
        ...
        switch (@typeInfo(ZigType)) {
            .@"struct" => |info| switch (info.layout) {
                ...
                .@"packed" => try emit_enum(&buffer, ZigType, c_name, &.{"padding"}),
                .@"extern" => try emit_struct(&buffer, ZigType, c_name),
            },
            .@"enum" => {
                ...
                try emit_enum(&buffer, ZigType, c_name, skip);
            },
            else => try buffer.print("typedef {s} {s}; \n\n", .{
    ...
    try stdout.interface.writeAll(buffer.items);
