...
pub fn main() !void {
    ...
    inline for (type_mappings) |type_mapping| {
        ...
        if (type_mapping.len == 3) {
            ...
            try output.writer.print(comments, .{});
            try output.writer.print("\n", .{});
        }
        ...
        switch (@typeInfo(ZigType)) {
            .@"struct" => |info| switch (info.layout) {
                ...
                .@"packed" => try emit_enum(&output.writer, ZigType, info, c_name, &.{"padding"}),
                .@"extern" => try emit_struct(&output.writer, info, c_name),
            },
            .@"enum" => |info| {
                ...
                try emit_enum(&output.writer, ZigType, info, c_name, skip);
            },
            else => try output.writer.print("typedef {s} {s}; \n\n", .{
    ...
    try stdout_writer.interface.writeAll(output.writer.buffered());
}
